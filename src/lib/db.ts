...
async function ensurePriceHistoryTable(db: any): Promise<void> {
  await db.prepare(`
...
export async function recordPrice(db: any, productId: string, price: string): Promise<void> {
  await ensurePriceHistoryTable(db);
  const id = crypto.randomUUID();
  const createdAt = new Date().toISOString();
  await db.prepare(
    'INSERT INTO price_history (id, product_id, price, created_at) VALUES (?, ?, ?, ?)'
  ).bind(id, productId, price || '').run();
}

export async function getPriceHistory(db: any, productId: string): Promise<PriceRecord[]> {
  await ensurePriceHistoryTable(db);
  const { results } = await db.prepare(
    'SELECT * FROM price_history WHERE product_id = ? ORDER BY created_at ASC'
  ).bind(productId).all();
...
export async function deletePriceHistory(db: any, productId: string): Promise<void> {
  await ensurePriceHistoryTable(db);
